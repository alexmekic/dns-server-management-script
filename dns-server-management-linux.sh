...
function add_reverse_zone {
	last_octet=$(ip addr show dev $main_int | awk '/inet.*brd/{print $2}' | cut -d "/" -f 1 | cut -d '.' -f 4)
	if [[ ! -z $4 ]]; then
...
function add_new_zone {
	while true; do
		...
		if [[ $forward_zone == "0" ]]; then
			return 1
		elif [ find /etc/bind/zones $forward_zone > /dev/null 2>&1 ]; then
                ...
                if [[ $zone_remove == "0" ]]; then
                        ...
                        if [[ -f $zone_remove ]]; then
                                for remove_domain in $(find . -type f -print0 | xargs -0 grep -l "$zone_remove"); do
                                        rm $remove_domain
					sed -i '/zone "'$remove_domain'"/,+4d' /etc/bind/named.conf.local
                                done
...
zone \"$reverse_zone\" {
	...
	echo "$forward_zone added as master zone"
	if ! grep allow-transfer /etc/bind/named.conf.local | grep [0-9] > /dev/null; then
		sed -i 's#allow-transfer#//allow-transfer#g' /etc/bind/named.conf.local
	fi
	echo "Done"
}
...
zone \"$reverse_ip.in-addr.arpa\" {
        type slave;
        file \"/etc/bind/zones/$reverse_ip.in-addr.arpa\";
        masters { $primary_ip; };
};" >> /etc/bind/named.conf.local
	echo "$forward_zone added as slave zone"
                                                break
                                        else
...
main_int=$(ip addr show | awk '/inet.*brd/{print $NF}')
main_ip=$(ip addr show dev $main_int | awk '/inet.*brd/{print $2}' | cut -d "/" -f 1)

if [ ! $main_int ]; then
	echo "ERROR: No active IP addresses found. Program aborted"
	exit 1;
fi
...
while true; do
	cd "/etc/bind/zones"
	echo ""
	echo "DNS Server Management for Debian Linux v1.1"
	echo ""
